...
{
    public class AmmoItemInTradeUI : AmmoItemInInventoryUI
    {
        ...
        public override void UpdateItem()
        {
            ...
            if (TradeSystemUI.instance.isBuyingOrDrawing && ammoItemInInventory.metadata.isOwned)
            {
                if (TradeSystemUI.instance.curMerchant != null && TradeSystemUI.instance.curMerchant.ID == ammoItemInInventory.metadata.ownerID)
        ...
        public override void OnClick(bool takeAll = false)
        {
            ...
            {
                Debug.Log("Taking item: " + base.ammoItemInInventory.item.ItemName);
                if (TradeSystemUI.instance.isMerchant)
                {
                    int adjustedGolds = Mathf.RoundToInt(base.ammoItemInInventory.item.Value * TradeSystemUI.instance.curMerchant.sellMultiplier);
                    ...
                    if (base.ammoItemInInventory.Amount <= 1)
                    {
                        ...
                        TradeSystemUI.instance.confirmTradePanel.Init(base.ammoItemInInventory, this, "Are you sure you want to buy 1 " + base.ammoItemInInventory.item.ItemName + " for " + adjustedGolds + " golds?");
                    ...
                    else if (base.ammoItemInInventory.item.isCumulable && base.ammoItemInInventory.Amount > 1)
                    {
                        ...
                        TradeSystemUI.instance.takeDepositItemsPanel.Init(ammoItemInInventory, this);
                    }
                }
                else
                {
            ...
            else // we're selling
            {
                ...
                if (TradeSystemUI.instance.isMerchant)
                {
                    int adjustedGolds = Mathf.RoundToInt(base.ammoItemInInventory.item.Value * TradeSystemUI.instance.curMerchant.buyMultiplier);

                    // if the amount is 1, add it one time
                    if (base.ammoItemInInventory.Amount <= 1)
                    {
                        TradeSystemUI.instance.confirmTradePanel.gameObject.SetActive(true);
                        TradeSystemUI.instance.confirmTradePanel.Init(base.ammoItemInInventory, this, "Are you sure you want to sell 1 " + base.ammoItemInInventory.item.ItemName + " for " + adjustedGolds + " golds?");
        ...
        public override void ConfirmButtonCumulableItem(int amount)
        {
            if (TradeSystemUI.instance.isBuyingOrDrawing)
            {
                if (TradeSystemUI.instance.isMerchant)
                {
                    int adjustedGolds = Mathf.RoundToInt((base.ammoItemInInventory.item.Value * TradeSystemUI.instance.curMerchant.sellMultiplier)) * amount;

                    TradeSystemUI.instance.takeDepositItemsPanel.CancelButton();

                    TradeSystemUI.instance.confirmTradePanel.gameObject.SetActive(true);
                    TradeSystemUI.instance.confirmTradePanel.Init(base.ammoItemInInventory, this, "Are you sure you want to buy " + amount + " " + base.ammoItemInInventory.item.ItemName + " for " + adjustedGolds + " golds?", true);
            ...
            else // we're selling
            {
                if (TradeSystemUI.instance.isMerchant)
                {
                    int adjustedGolds = Mathf.RoundToInt((base.ammoItemInInventory.item.Value * TradeSystemUI.instance.curMerchant.buyMultiplier)) * amount;

                    TradeSystemUI.instance.takeDepositItemsPanel.CancelButton();

                    TradeSystemUI.instance.confirmTradePanel.gameObject.SetActive(true);
                    TradeSystemUI.instance.confirmTradePanel.Init(base.ammoItemInInventory, this, "Are you sure you want to sell " + amount + " " + base.ammoItemInInventory.item.ItemName + " for " + adjustedGolds + " golds?", true);
        ...
        {
            // base price
            int totalGoldsAmount = (base.ammoItemInInventory.item.Value * _amount);

            if (_action == ConfirmTradeAction.BuyFromMerchant)
            {
                totalGoldsAmount = Mathf.RoundToInt(base.ammoItemInInventory.item.Value * TradeSystemUI.instance.curMerchant.sellMultiplier) * _amount;
                ...
                if (Inventory.PlayerInventory.GetItemCount("Gold001") >= totalGoldsAmount)
                {
                    ...
                    base.ammoItemInInventory.metadata.Clear();

                    Inventory.PlayerInventory.AddItem(base.ammoItemInInventory.item, base.ammoItemInInventory.metadata, _amount);
                    ...
                    TradeSystemUI.instance.curMerchant.inventory.RemoveItem(base.ammoItemInInventory, _amount);
                    ...
                    if (base.ammoItemInInventory.Amount <= 0)
                    {
            ...
            {
                totalGoldsAmount = Mathf.RoundToInt(base.ammoItemInInventory.item.Value * TradeSystemUI.instance.curMerchant.buyMultiplier) * _amount;
                ...
                if (TradeSystemUI.instance.curMerchant.inventory.GetItemCount("Gold001") >= totalGoldsAmount)
                {
                    ...
                    TradeSystemUI.instance.curMerchant.inventory.AddItem(base.ammoItemInInventory.item, base.ammoItemInInventory.metadata, _amount);
                    Inventory.PlayerInventory.RemoveItem(base.ammoItemInInventory, _amount);
