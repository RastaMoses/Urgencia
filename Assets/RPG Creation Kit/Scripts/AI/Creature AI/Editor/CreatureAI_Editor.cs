...
{
    public class CreatureAI_Editor : EditorWindow
    {
        bool isReady = false;
        ...
        List<CreatureAI> allAI = new List<CreatureAI>();
        ...
        public CreatureAI RckAIPrefabComponent; // The RckAI script of the RckAIPrefab
        public GameObject RckAIGfx;

        string previouEntityID = "";


        [MenuItem("RPG Creation Kit/CreatureAI Editor")]
        public static void ShowWindow()
        {
            EditorWindow thisWindow = GetWindow(typeof(CreatureAI_Editor));

            // Set Title
            Texture icon = AssetDatabase.LoadAssetAtPath<Texture>(EditorIconsPath.CreatureAIEditorIcon);
            GUIContent titleContent = new GUIContent("CreatureAI Editor", icon);
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Creature AI", HeaderStyle, GUILayout.MaxWidth(165.0f), GUILayout.ExpandWidth(false));

            if (GUILayout.Button(new GUIContent(refreshButtonIcon, "Refreshes the Creatures AI list."), GUILayout.MaxWidth(32)))
                RefreshRckAIList();
            ...
            if (!isCreatingNewRckAI)
            {
                ...
                if (clickArea.Contains(current.mousePosition) && current.type == EventType.ContextClick)
                {
                    ...
                    menu.AddDisabledItem(new GUIContent("Creature AI"));
                    menu.AddSeparator("");
                ...
                void CreateNew()
                {
                    ...
                    CreatureAI newAIComponent = newAI.GetComponent<CreatureAI>();

                    newAI.name = "[CreatureAI] NewCreatureAI";

                    newAIComponent.entityID = "_New_CreatureAI";
                    newAIComponent.entityName = "New CreatureAI";

                    string path = AssetDatabase.GenerateUniqueAssetPath(RCKSettings.EDITOR_CREATUREAI_SAVE_LOCATION + "[CreatureAI] NewCreatureAI.prefab");
                ...
                void DuplicateSelected()
                {
                    if (RckAIAsset != null)
                    {
                        ...
                        CreatureAI aiComponent = newAI.GetComponent<CreatureAI>();

                        aiComponent.entityID += "Copy";

                        PrefabUtility.SaveAsPrefabAsset(newAI, AssetDatabase.GenerateUniqueAssetPath(RCKSettings.EDITOR_CREATUREAI_SAVE_LOCATION + "[CreatureAI] " + aiComponent.entityID + ".prefab"));
            ...
            if (GUILayout.Button("CreatureAI", GUILayout.Width(100), GUILayout.Height(20)))
            {
                    ...
                    case CurrentWindow.Model:
                        ...
                        if (windowEditor != null)
                        {
                            ...
                            if(!RckAIPrefabComponent.validAssignedModel)
                                EditorGUILayout.HelpBox("ERROR! Selected 3D Model is not correctly setup to be a Creature AI GFX. Behaviour is undefined", MessageType.Error);
                        }
                        else
                        {
                            ...
                            EditorGUILayout.HelpBox("There is no Model associated with this Creature, reference a 3D Model.", MessageType.Warning);
                        }
            ...
            if (GUILayout.Button("SAVE", ButtonStyle))
            {
                ...
                RckAIAsset.name = "[CreatureAI] " + RckAIPrefabComponent.entityID;
                AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(RckAIAsset), "[CreatureAI] " + RckAIPrefabComponent.entityID);
        ...
        private void OnSelectedRckAIChanges()
        {
            ...
            if (RckAIPrefab != null)
            {
                RckAIPrefabComponent = RckAIPrefab.GetComponent<CreatureAI>();
        ...
        private void RegisterPersistentReference()
        {
            ...
            if (!editorPRef.refs.TryGetValue(RckAIPrefabComponent.entityID, out pref))
            {
                ...
                instance.GetComponent<CreatureAI>().enabled = false;
        ...
        private void GenerateNewGFX()
        {
            ...
            if(RckAIPrefabComponent.assignedModel != null)
            {
                ...
                {
                    Debug.LogError("ERROR! Selected 3D Model \"" + RckAIPrefabComponent.assignedModel + "\" for the CreatureAI with ID \"" + RckAIPrefabComponent.entityID + "\" was not correctly setup. Behaviour is undefined.");
        ...
        public static List<CreatureAI> GetAllRckAI()
        {
            string[] allAssets = Directory.GetFiles(RCKSettings.EDITOR_CREATUREAI_SAVE_LOCATION, "*.prefab", SearchOption.AllDirectories);
            ...
            for (int i = 0; i < allAssets.Length; i++)
            {
                ...
                {
                    thisAI = (assetObject as GameObject).GetComponent<CreatureAI>();

                    if (thisAI != null && thisAI.entityID != "DEFAULT_CREATURE_ID")
                        allAI.Add((assetObject as GameObject).GetComponent<CreatureAI>());
                }
        ...
        private static int SortByID(CreatureAI a1, CreatureAI a2)
        {
