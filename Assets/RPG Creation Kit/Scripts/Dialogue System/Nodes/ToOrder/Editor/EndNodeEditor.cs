...
namespace RPGCreationKit.DialogueSystem
{
    ...
    public class EndNodeNodeEditor : NodeEditor
    {
        private EndNode lineNode;

        public MonoScript[] allResultScripts;

        public override void OnBodyGUI()
        {
            if (lineNode == null) lineNode = target as EndNode;
            if (allResultScripts == null) allResultScripts = NodesHelper.GetAllResultScripts<ResultScript>();

            // Update serialized object's representation
            serializedObject.Update();

            NodeEditorGUILayout.PropertyField(serializedObject.FindProperty("endNode"));
