...
public class Gamepad_InventoryTabs : MonoBehaviour
{
    ...
    void Update()
    {
        if (RckInput.isUsingGamepad && RckPlayer.instance.input.currentActionMap.name == "InventoryUI")
        {
            ...
            {
                if ((InventoryUI.instance.selectedTab == Enum.GetValues(typeof(InventoryTabs)).Cast<InventoryTabs>().Last()))
                    tabs[0].isOn = true;
                else
                    tabs[(int)InventoryUI.instance.selectedTab + 1].isOn = true;
            }
            else if (RckPlayer.instance.input.currentActionMap.FindAction("LeftInventoryTab").triggered)
            {
                if ((InventoryUI.instance.selectedTab == 0))
