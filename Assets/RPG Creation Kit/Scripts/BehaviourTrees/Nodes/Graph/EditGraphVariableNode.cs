...
namespace RPGCreationKit.BehaviourTree
{
    ...
    [CreateNodeMenu("RPGCK_BehaviourTree/Graph/Edit BTVariable", order = 1)]
    [System.Serializable]
    public class EditGraphVariableNode : BTNode
    {
        public BTVariable btVariable;

        public BTVariableEdit editType;
        ...
        public override NodeState Execute(RckAI eAI)
        {
            ...
            switch (editType)
            {
                case BTVariableEdit.Add:
                    switch (instantValue.parameterType)
                    {
                        ...
                        case BTParameterType.INT:
                            btVariableRuntime.SetValue(((BT_Int)btVariableRuntime).value + instantValue.intValue);
                            break;

                        case BTParameterType.FLOAT:
                            btVariableRuntime.SetValue(((BT_Float)btVariableRuntime).value + instantValue.floatValue);
                            break;
                    }
                    break;

                case BTVariableEdit.Divide:
                    switch (instantValue.parameterType)
                    {
                        case BTParameterType.BOOL:
                            btVariableRuntime.SetValue(instantValue.boolValue);
                ...
                default:
                    Debug.Log("CASE NOT WRITTEN YET! 001x01");
                    break;
            }
