    ...
    public class BookItemInLootingUI_DrawDeposit : BookItemInInventoryUI
    {
            ...
            if (LootingInventoryUI.instance.isDrawing)
            {
                ...
                if (base.bookItemInInventory.Amount <= 1)
                {
                    ...
                    if (LootingInventoryUI.instance.isPickPocketing)
                    {
                        ...
                        float chance = RCKFunctions.CalculatePickpocketChance(dotProduct > 0, bookItemInInventory.item.Weight, Player.RckPlayer.instance.playerAttributes.attributes.Dexterity, bookItemInInventory.isEquipped);
                    ...
                    // Check steal
                    if (!LootingInventoryUI.instance.isPickPocketing && RCKFunctions.CheckPlayerStealsItem(base.bookItemInInventory))
                    {
                        // Close looting ui
                        LootingInventoryUI.instance.CloseUI();
                    }
        ...
        public override void ConfirmButtonCumulableItem(int amount)
        {
            if (LootingInventoryUI.instance.isDrawing)
            {
                ...
                {
                    float dotProduct = Vector3.Dot(LootingInventoryUI.instance.pickPocketingAI.transform.position - Player.RckPlayer.instance.transform.position, LootingInventoryUI.instance.pickPocketingAI.transform.forward);
                    float chance = RCKFunctions.CalculatePickpocketChance(dotProduct > 0, bookItemInInventory.item.Weight, Player.RckPlayer.instance.playerAttributes.attributes.Dexterity, bookItemInInventory.isEquipped, amount);
