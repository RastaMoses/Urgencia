...
namespace UnityStandardAssets.Cameras
{
    public class ProtectCameraFromWallClip : MonoBehaviour
    {
        ...
        public List<string> dontClipList;
        ...
        private void LateUpdate()
        {
            ...
            {
                if ((!cols[i].isTrigger) &&
                    !(cols[i].attachedRigidbody != null && dontClipList.Contains(cols[i].attachedRigidbody.tag)) && !cols[i].CompareTag("Player"))
                {
            ...
            for (int i = 0; i < m_Hits.Length; i++)
            {
                ...
                if ((m_Hits[i].distance < nearest && (!m_Hits[i].collider.isTrigger) &&
                    !(m_Hits[i].collider.attachedRigidbody != null &&
                      dontClipList.Contains(m_Hits[i].collider.attachedRigidbody.tag))) && !m_Hits[i].transform.gameObject.CompareTag("Player"))
                {
